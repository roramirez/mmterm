...
impl App {
        ...
        match state {
            ElementState::Pressed => {
                if let Some((mx, my)) = self.state.mouse_pos {
                    match self.state.click_count(mx, my) {
                        3 => self.select_line_at(mx, my),
                        2 => self.select_word_at(mx, my),
                        _ => self.start_mouse_selection(mx, my),
                    }
                }
            }
    ...
    fn viewport_scroll(&mut self, lines: f32) {
        self.state.viewport_scroll(lines);
        if let Some(w) = &self.window {
            w.request_redraw();
        }
    }
