...
- OSC 133 prompt navigation: `[` / `]` in Normal mode jump the viewport between shell prompts, `y` copies the last command's output to the clipboard, and prompt rows get a subtle left-edge marker
...
- `totoro` bundled theme: Studio Ghibli–inspired palette with a deep forest-night background, warm cream foreground, and dusty greens, acorn ambers, and umbrella blues drawn from *My Neighbor Totoro*
...
### Fixed
- tab rename prompt now repaints on every keystroke; previously typed characters appeared only after the next unrelated redraw
...
- refactor app_event.rs: extract separator_at_pixel() and active_grid_params() helpers; reduce handle_keyboard_input nesting and eliminate duplicate separator lookup in try_start_separator_drag
- refactor renderer/overlays.rs: extract draw_palette_entry() helper; collapse 43-line draw_command_palette loop body to 5 lines
